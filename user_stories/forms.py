...
class FormRestoreUserStoryHistory(forms.Form):
    ...
    def __init__(self, project_id, *args, **kwargs):
        ...
        self.fields['code'] = forms.CharField(max_length=100, label='Codigo',widget=widgets.TextInput(attrs={'readonly': 'readonly'}))  # Codigo del US
        ...
        self.fields['sprint'] = forms.ModelChoiceField(
            queryset=ProjectUseCase.filter_user_story_type_by_project(project_id), label='Sprint',
            empty_label='Seleccione un tipo',
            widget=widgets.SelectInput()
        )
        self.fields['sprint_member'] = forms.ModelChoiceField(
            queryset=ProjectUseCase.filter_user_story_type_by_project(project_id), label='Miembro asignado',
            empty_label='Seleccione un tipo',
            widget=widgets.SelectInput()
        )